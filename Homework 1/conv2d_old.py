...
def conv1d(image, kernel, mode_conv="same", mode_padding="zero"):
    if mode_conv == "full":
    ...
    for i in range(length0, image_padding.shape[0] - length0):
        for j in range(length1, image_padding.shape[1] - length1):
            print(image_padding[i - length0:i + length0 + 1, j - length1:j + length1 + 1].shape, kernel.shape, i, j, image_conv.shape, image_padding.shape)
